...
describe("formatCurrency", () => {
  // The default currency is USD, so an amount with no explicit currency
  // formats in USD ($). Pass a currency to get anything else.
  ...
  it("still honours an explicit currency", () => {
    expect(formatCurrency(450, "NGN")).toBe("\u20a6450");
  });
});
