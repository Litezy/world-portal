...
export function useInvitePassportApplicant(id: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: [...passportKeys.detail(id), "invite"],
    mutationFn: (input: InviteApplicantInput) =>
      internalApi
        .post<ApiResponse<PassportApplication>>(`/admin/passports/${id}/invite`, input)
        .then((r) => r.data),
    onSuccess: (passport) => {
