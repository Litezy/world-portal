 ...
 * ring — E-Embassy blue here, WorldStreet's gold on the hub.
 *
 * Copied from the WorldStreet app. Local changes: colours, import path, and
 * the rect is no longer reset inside the effect (React Compiler lint).
 *
...
export default function VividSpotlight() {
  ...
  useEffect(() => {
    // Inactive renders nothing (see the return below). The last rect is kept
    // on purpose: the next spotlight glides from it rather than popping in.
    if (!activeId) return;
  ...
  return (
    <div
      ...
      style={{
        ...
        width: rect.width,
        height: rect.height,
        // Veil + brand ring in one paint: ink-toned veil, brand-blue ring.
        boxShadow:
          "0 0 0 200vmax rgba(5, 13, 36, 0.62), 0 0 0 2px rgba(0, 80, 192, 0.7), 0 0 24px 2px rgba(0, 184, 248, 0.25)",
      }}
    />
  );
}
