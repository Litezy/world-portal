 ...
 *
 * Copied from Xtreme/WorldStreet. Local changes: E-Embassy's colours, and the
 * render loop's refs are synced in an effect rather than during render.
 *
...
/**
 * E-Embassy's palette on the same field: cyan veins (#00B8F8, the secondary
 * swoosh) through brand blue (#0050C0) on deep ink — a dark object that reads
 * on this app's light pages. WorldStreet's hub uses ember on near-black.
 */
const TINT: [number, number, number] = [0.0, 0.72, 0.97];
const DEEP: [number, number, number] = [0.0, 0.2, 0.52];
const BASE: [number, number, number] = [0.02, 0.05, 0.14];
    ...
    >
      {glFailed && (
        ...
        <span
          ...
          style={{
            background:
              "radial-gradient(circle at 46% 40%, rgba(0,184,248,0.9) 0%, rgba(0,80,192,0.85) 46%, rgba(5,13,36,0.9) 72%, transparent 96%)",
          }}
        />
      )}
      <canvas
