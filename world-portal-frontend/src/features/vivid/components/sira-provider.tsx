 ...
 *      per turn).
 *
 * Copied from Xtreme (components/vivid/sira-provider.tsx) — the plumbing is
 * identical on every WorldStreet service. Local changes: the type imports, and
 * a locked (402) session goes to WorldStreet's paywall via WORLDSTREET_URL.
 *
 * Tool bridge: a `tool_call` {id, name, args} runs the matching
 * features/vivid/functions tool — server-context ones through /api/vivid/function,
...
import type { VividAgentState, VoiceFunctionConfig } from "@/features/vivid/types";
...
export function SiraVividProvider({
  ...
  const playChunk = useCallback(
    (b64: string, mime: string | undefined) => {
      ...
      const rate =
        Number(mime?.match(/rate=(\d+)/)?.[1]) ||
        sessionRef.current?.output_audio?.sample_rate ||
        24000;
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      capture.port.onmessage = (e: MessageEvent<ArrayBuffer>) => {
        if (!live() || !readyRef.current || socket.readyState !== WebSocket.OPEN)
          return;
        ...
        if (!playing) {
          if (
            db > Math.max(SPEECH_DB_FLOOR, noiseDbRef.current + SPEECH_DB_ABOVE_NOISE)
          ) {
      ...
      socket.onmessage = (e) => {
        ...
        switch (event.type) {
          ...
          case "tool_call": {
            ...
            const { id, name } = event;
            setState("processing");
            void runTool(functionsRef.current, name, event.args ?? {}).then(
              (result) => {
      ...
      socket.onclose = (e) => {
        ...
        if (e.code !== 1000 && e.code !== 1005)
          console.warn(`[vivid] sira: closed ${e.code} ${e.reason}`);
        teardown("idle");
      };
    } catch (e) {
      if (!live()) return;
      ...
      if (code === "locked") {
        ...
        window.location.assign(new URL("/vivid", WORLDSTREET_URL).toString());
        return;
      }
  ...
  return (
    <SiraVividContext.Provider value={ctxValue}>{children}</SiraVividContext.Provider>
  );
}
