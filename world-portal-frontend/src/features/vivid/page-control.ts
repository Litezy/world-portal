 ...
 *
 * Copied from the WorldStreet app (lib/vivid-page-control.ts) so Vivid behaves
 * the same on every WorldStreet service. Local changes: no Lenis (E-Embassy
 * scrolls natively), no "use client" (the server reads the tool list too, and
 * nothing here touches the DOM at import time), and semicolons.
 *
 * ── Architecture ────────────────────────────────────────────────────────────
...
export function listTargets(): VividTarget[] {
  ...
  document.querySelectorAll(`[${TARGET_ATTR}]`).forEach((el) => {
    ...
    out.push({
      ...
      label: labelOf(el),
      kind: kindOf(el),
      guarded: el.hasAttribute(GUARD_ATTR),
    });
  });
  return out;
}
