/**
 * WorldStreet's shared ecosystem blurb for Vivid, copied verbatim from the
 * WorldStreet app (lib/vivid-worldstreet-context.ts) so Vivid describes the
 * platform the same way on every service. Update it there first, then here.
 */
...
- Identity verification (KYC) is required before a user can withdraw or transfer money out. Deposits, conversions and trading never require it. It takes about two minutes at /verification — NIN or BVN plus a selfie, handled by Prembly. If someone hits a blocked withdrawal, send them to /verification; do not suggest workarounds, because there are none.
...
- Crypto prices are live market data. Forex rates are verified by a live web check at the moment they are asked for, and always come with an as-of time.
- Dollar → Naira conversion is LIVE: the "convert" panel moves dollars from the Dollar Account's available balance into the naira wallet. The dollars leave instantly; the naira arrives in the user's bank-transfer wallet account as a real transfer, usually under a minute — it is NOT instant, so if someone converts and doesn't see the naira yet, tell them it's on the way and shows as "Dollar → Naira" (Processing) in Recent activity. A small spread is built into the rate shown, only settled (available) dollars can be converted, and there is a $500-per-day limit. If a transfer fails, the dollars are returned automatically.
- Naira → Dollar conversion (the opposite direction) is TEMPORARILY UNAVAILABLE while it is being reworked. Do not offer it and do not improvise it via the convert panel — that panel only goes dollars-to-naira. If someone asks to turn naira into dollars, say that direction is briefly offline; they can still fund the Dollar Account by card, payable in naira. Past conversions still appear in Recent activity as "Naira → Dollar"; you may read those back.
