...
/**
 * Sira — WorldStreet's voice session broker at sira.vividintelligence.tech.
 *
 * Copied from Xtreme (lib/vivid/sira.ts), which copied it from the WorldStreet
 * app; only the env access changed (`serverEnv()`). Server-only: nothing under
 * src/features or src/components may import it.
 *
 ...
 * (see src/server/vivid/voice-instructions.ts); tool calls arrive on the session's
...
function apiUrl(): string {
  return (serverEnv().SIRA_API_URL || DEFAULT_API_URL).replace(/\/+$/, "");
}

export function siraConfigured(): boolean {
  return Boolean(serverEnv().SIRA_API_KEY);
}

function headers(): HeadersInit {
  const key = serverEnv().SIRA_API_KEY;
 ...
 */
export async function createSiraSession(
  input: CreateSiraSessionInput = {},
): Promise<SiraSession> {
  const base = {
    provider: "gemini",
    voice: isSiraVoice(input.voice) ? input.voice : DEFAULT_SIRA_VOICE,
  };
  const extras = {
  ...
  if ((res.status === 400 || res.status === 422) && Object.keys(extras).length) {
    console.warn(
      `[vivid/sira] mint refused with instructions/tools (${res.status}); retrying without`,
    );
    res = await mint(base);
  }
  if (!res.ok) {
    ...
    throw new SiraError(
      `Sira refused to mint a session (${res.status}): ${body.slice(0, 300)}`,
      res.status,
    );
  }
...
export async function deleteSiraSession(id: string): Promise<void> {
  ...
  if (!res.ok && res.status !== 404) {
    throw new SiraError(
      `Sira refused to end session ${id} (${res.status})`,
      res.status,
    );
  }
}
